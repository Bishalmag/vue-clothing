...
const removeFromFavorites = item => toggleFavorite(item)
      ...
      <div v-else class="favorites-grid">
        <div v-for="item in favorites" :key="item.id" class="favorite-item">
          <div class="item-image">
            ...
            <button @click="removeFromFavorites(item)" class="remove-favorite-btn">❤️</button>
          </div>
          <div class="item-info">
            <h3>{{ item.name }}</h3>
          ...
          <button class="add-to-cart-btn" @click="handleAddToCart(item)">Add to Cart</button>
        </div>
      </div>
    </div>
  </div>
