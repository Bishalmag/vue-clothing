...
function validateForm() {
  ...
  else if (!/\S+@\S+\.\S+/.test(form.email)) { errors.email = 'Enter a valid email address'; isValid = false }
  if (!form.password) { errors.password = 'Password is required'; isValid = false }
  else if (form.password.length < 8) { errors.password = 'Min 8 characters'; isValid = false }
  else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(form.password)) {
    errors.password = 'Needs uppercase, lowercase, and numbers'; isValid = false
  }
  if (!form.confirmPassword) { errors.confirmPassword = 'Confirm password'; isValid = false }
  else if (form.password !== form.confirmPassword) { errors.confirmPassword = 'Passwords do not match'; isValid = false }
  if (!form.acceptTerms) { alert('You must accept the Terms and Policy.'); isValid = false }
...
async function handleSignUp() {
  ...
  try {
    // Session-based signup (Laravel will manage cookies)
    ...
    // Optionally, fetch logged-in user
    const userRes = await api.get('/me')
    console.log('Signed up user:', userRes.data)
    ...
    if (err.response?.data?.errors) {
      ...
      errors.email = err.response?.data?.message || 'Sign up failed'
    }
  } finally {
    loading.value = false
  }
